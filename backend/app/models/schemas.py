...
class RouteRequest(BaseModel):
    """路线规划请求"""
    origin_address: str = Field(...,description="起始地址",example="北京市朝阳区阜通东大街6号")
...
class Attraction(BaseModel):
    ...
    rating: Optional[str] = Field(default=None,description="景点评分")
    image_url: Optional[str] = Field(default=None,description="景点图片URL")
    ticket_price: int = Field(default=0,description="门票票价（元）")
...
class Meal(BaseModel):
    ...
    estimated_cost: int = Field(default=0,description="预估费用（元）",ge=0)

class Hotel(BaseModel):
    """酒店信息"""
    name: str = Field(...,description="酒店名称")
    address: str = Field(default="",description="酒店地址")
    location: Optional[Location] = Field(default=None,description="酒店经纬度坐标")
...
class DayPlan(BaseModel):
    ...
    attractions: List[Attraction] = Field(default_factory=list,description="景点信息")
